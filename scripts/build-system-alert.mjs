...
const NORMALISE = [
  ...
  '// ─── END INLINED MODULE ─────────────────────────────────────────────────────────────────────',
  '',
  '// THE GUARD. C1 failure inputs retain their strict normalisation. C2 recovery and data-warning',
  '// events additionally pass through persisted transition state, so a healthy probe without a',
  '// known failure and an unchanged warning are both silent.',
  'const raw = $input.first().json || {};',
  'const state = $getWorkflowStaticData("global");',
  'const verdict = SAE.transition(raw, state);',
  'return [{ json: {',
  '  emit: verdict.emit === true ? 1 : 0,',
  '  silent_reason: String(verdict.reason || ""),',
  '  alert_kind: String(verdict.kind || ""),',
...
const BUILD_ALERT = [
  ...
  '// ─── END INLINED MODULE ─────────────────────────────────────────────────────────────────────',
  '',
  '// One rendering authority for the three operational event types. The C1 failure branch remains',
  '// byte-identical in presentation; the two C2 branches use the already-approved renderers.',
  'const v = $("Normalise Alert Event").first().json || {};',
  'const e = $("Normalise Alert Event").first().json.event || {};',
  'let alert_html = "";',
  'if (v.alert_kind === "recovered") {',
  '  alert_html = LA.renderSystemRecovered({',
  '    problem: String(e.problem || ""), evidence: String(e.evidence || ""),',
  '    recoveredAt: String(e.occurred_at || ""), ownerActionRequired: e.owner_action_required === true,',
  '    ownerAction: String(e.owner_action || "")',
  '  });',
  '} else if (v.alert_kind === "data_warning") {',
  '  alert_html = LA.renderDataIntegrity({',
  '    items: Array.isArray(e.items) ? e.items : [], checkedAt: String(e.checked_at || ""),',
  '    ownerAction: String(e.owner_action || "")',
  '  });',
  '} else {',
  '  alert_html = LA.renderSystemAlert({',
  '    workflowName: String(e.workflow_label || ""), nodeName: String(e.stage || ""),',
  '    operation: String(e.operation || ""), sideEffectClass: String(e.side_effect_class || ""),',
  '    errorCode: String(e.error_code || ""), retryable: e.retryable === true,',
  '    routeIdentity: String(e.route_identity || ""), errorClass: "", message: "", executionId: ""',
  '  });',
  '}',
