...
//
// 2. TWO NODES between `Build Session Row` and `Save Bot Session`:
//
//       Build Session Row -> Project Cycle -> Cycle Projection Guard -> Save Bot Session
//
//    `Project Cycle` upserts (telegram_user_id -> cycle_id) into the n8n Data Table
//    `MiniApp_Cycle_Projection` on EVERY turn, BEFORE the session is persisted. Ordering is the
//    whole point: the Gateway must see the new cycle or nothing — never the old one after a
//    rotation. On a rotation turn a failed projection write ABORTS the turn (the guard throws,
//    the Error Monitor alerts, Bot_Sessions keeps the old cycle, the projection keeps the old
//    cycle: consistent). On any other turn a failed write is tolerated: the projection is at
//    worst missing, and a missing projection makes the Gateway refuse, never resume.
//
...
if (isMain) {
  ...
  ok('exactly ' + PREMIUM_SESSION + ' rewritten, ' + PROJECT_NODE + ' + ' + GUARD_NODE + ' added, one edge split, nothing else');
  ...
  if (e !== PROJECT_NODE) { bad(BUILD_ROW + ' feeds ' + e); } else { ok(BUILD_ROW + ' -> ' + PROJECT_NODE + ' -> ' + GUARD_NODE + ' -> ' + SAVE_SESSION); }
