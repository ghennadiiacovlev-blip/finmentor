...
const BUILD_SESSION_CODE = [
  ...
  "const crypto = require('crypto');",
  "const c = $('Claim Verdict').first().json;",
  "const C1_FIELDS = ['company_name','business_activity','role','turnover_band','objective','problem','problem_free_text','desired_outcome','desired_outcome_free_text','current_setup','decision_horizon','documents','contact_channel','contact_value','important_context','locale','contact_name'];",
  "function c1Projection(value) { const raw = String(value || ''); if (raw.slice(0, 3) !== 'C1:') return {}; try { const p = JSON.parse(raw.slice(3)); return p && p.v === 1 ? p : {}; } catch (e) { return {}; } }",
  "function c1Field(value, source, confirmed, at) { const present = value !== null && value !== undefined && value !== ''; return { value: present ? value : null, source: present ? source : null, confirmed: present && confirmed === true, at: present && confirmed === true ? at : null }; }",
  "function c1Draft(row, cycleId, nowIso) {",
  "  const p = c1Projection(row && row.cycle_reset); const fields = {}; for (const name of C1_FIELDS) fields[name] = c1Field(null, null, false, null);",
  "  const extracted = p.extracted && typeof p.extracted === 'object' ? p.extracted : {}; const confirmed = p.context_confirmed === true; const source = confirmed ? 'user_confirmed' : 'ai_inferred';",
  "  for (const name of ['company_name','business_activity','role','turnover_band','objective']) { const value = String(extracted[name] || '').trim(); if (value) fields[name] = c1Field(value, source, confirmed, nowIso); }",
  ...
  "});",
  "const projection = proj[0] || null;",
  "const cycleId = projection ? String(projection.cycle_id) : '';",
  ...
  "  draft_json: JSON.stringify(c1Draft(projection, cycleId, now.toISOString()))",
  "} }];"
].join('\n');
