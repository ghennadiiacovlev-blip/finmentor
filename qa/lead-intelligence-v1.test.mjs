...
import { NIAGARA_AI, NIAGARA_BRIEF, NIAGARA_CLIENT_DRAFT, NIAGARA_CONTEXT, NIAGARA_ROW } from './fixtures/lead-intelligence-fixtures.mjs';
...
const editHtml = RENDER.renderOwnerBriefPage({ brief: NIAGARA_BRIEF, row: NIAGARA_ROW, client_draft: NIAGARA_CLIENT_DRAFT, auth, mode: 'edit' });
...
const previewHtml = RENDER.renderOwnerBriefPage({ brief: NIAGARA_BRIEF, row: NIAGARA_ROW, client_draft: NIAGARA_CLIENT_DRAFT, auth, mode: 'preview' });
...
const saved = ACTIONS.handleOwnerAction({ row: NIAGARA_ROW, body: editBody, now: '2026-09-10T07:00:00.000Z' });
...
const approved = ACTIONS.handleOwnerAction({ row: { ...NIAGARA_ROW, review_status: 'OWNER_EDITED' }, body: { action: 'approve' }, now: '2026-09-10T07:05:00.000Z' });
...
const repeatApprove = ACTIONS.handleOwnerAction({ row: { ...NIAGARA_ROW, review_status: 'CLIENT_READY' }, body: { action: 'approve' } });
...
const tgBrief = { ...NIAGARA_BRIEF, contact: LI.buildReachability({ preferred_contact_channel: 'telegram', telegram: '551662084', source_channel: 'telegram_premium' }) };
const nreq = NOTIFY.clientNotification({ row: { ...NIAGARA_ROW, review_status: 'CLIENT_READY' }, brief: tgBrief, client_result_url: 'https://app.finmentor.test/result' });
...
check('Niagara website route cannot auto-notify', NOTIFY.clientNotification({ row: { ...NIAGARA_ROW, review_status: 'CLIENT_READY' }, brief: NIAGARA_BRIEF, client_result_url: 'https://app.finmentor.test/result' }).reason === 'VERIFIED_TELEGRAM_ROUTE_UNAVAILABLE');
...
const manual = ACTIONS.handleOwnerAction({ row: { ...NIAGARA_ROW, review_status: 'CLIENT_READY' }, body: { action: 'manual_notify', notification_channel: 'email' }, now: '2026-09-10T07:30:00.000Z' });
