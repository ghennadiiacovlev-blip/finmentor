...
const pages = {
  ...
  // How FINMENTOR works (steps, working contour) moved from the homepage to the owner hub.
  owner: visible(read('owner.html')), roOwner: visible(read('ro/owner.html')),
...
check('consultation routes deeper work before start without defensive framing', () => assert(has(pages.consult, ['Когда нужен другой формат', 'сфокусирована на одном заранее определённом решении', 'более глубокого анализа компании', 'финансовой модели', 'внедрения', 'до начала работы']) && has(pages.roConsult, ['Când este necesar un alt format', 'unei singure decizii definite în prealabil', 'analiză mai aprofundată a companiei', 'model financiar', 'implementare', 'înainte de începerea colaborării']) && !/Границы консультации|Limitele consultației/i.test(pages.consult + pages.roConsult), 'premium scope routing missing'));
...
check('CFO AI Control starts at EUR 3,500/month with value-based involvement', () => assert(/CFO AI Control[\s\S]*?3 500 € \/ месяц/.test(pages.monthly) && /CFO AI Control[\s\S]*?3 500 € \/ lună/.test(pages.roMonthly) && has(pages.monthly, ['Регулярные CFO-разборы и управленческие брифы в согласованном ритме', 'Более высокий уровень вовлечённости CFO в согласованном рабочем контуре']) && has(pages.roMonthly, ['Analize CFO și sinteze manageriale regulate, într-un ritm convenit', 'Un nivel mai ridicat de implicare a CFO-ului în circuitul de lucru convenit']) && !/12[–-]16|часов в месяц|ore pe lună|Еженедельный финансовый разбор|Еженедельный бизнес-бриф|Analiză financiară săptămânală|Sinteză de business săptămânală/i.test(pages.monthly + pages.roMonthly), 'AI price or value-based scope mismatch'));
...
check('Health Check routes deeper scope to a separate project', () => assert(has(pages.health, ['Когда нужен другой формат', 'аудита', 'глубокой финансовой модели', 'внедрения системы', 'отдельный проект']) && has(pages.roHealth, ['Când este necesar un alt format', 'audit', 'model financiar aprofundat', 'implementarea unui sistem', 'proiect separat']) && !/Граница диагностики|Limita diagnosticului/i.test(pages.health + pages.roHealth), 'diagnostic scope routing missing'));
...
check('PREMIUM SELF-TEST Q3 — Partner controls decisions and execution', () => assert(has(pages.home + ' ' + pages.owner, ['Сопровождение и контроль исполнения', 'Контроль решений и исполнения', 'задачи с ответственным, сроком и статусом', 'следующем CFO-цикле']) && has(pages.roHome + ' ' + pages.roOwner, ['Parteneriat și controlul execuției', 'Controlul deciziilor și al execuției', 'sarcini cu responsabil, termen și stadiu']), 'execution-control cycle missing'));
check('PREMIUM SELF-TEST Q4 — remote work stays controlled between meetings', () => assert(has(pages.owner, ['Совместный рабочий контур', 'Удалённый формат без потери управляемости', 'прозрачный статус исполнения']) && has(pages.roOwner, ['Circuit comun de lucru', 'Lucru la distanță fără pierderea controlului', 'stadiul execuției']), 'remote control model missing'));
check('PREMIUM SELF-TEST Q5 — client participation is a professional partnership', () => assert(has(pages.owner, ['Результат строится совместно', 'команда клиента обеспечивает необходимые данные', 'исполнение согласованных решений']) && has(pages.roOwner, ['Rezultatul se construiește împreună', 'echipa clientului asigură datele necesare', 'executarea deciziilor convenite']), 'partnership model missing'));
...
check('premium addendum — limited engagements preserve direct CFO involvement', () => assert(has(pages.owner, ['ограниченное количество проектов одновременно', 'прямую вовлечённость CFO', 'контроль исполнения']) && has(pages.roOwner, ['număr limitat de proiecte', 'implicarea directă a CFO-ului', 'controlul execuției']), 'capacity signal missing'));
