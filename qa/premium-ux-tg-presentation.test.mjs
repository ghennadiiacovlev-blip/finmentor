...
// ---------------------------------------------------------------- the Russian customer

// P1-01. There is no longer a "legacy path" to leave untouched: the premium conversation IS the
// customer conversation, spliced into `Build Bot Response` itself, because thirteen downstream
// nodes read that node by name. The property that used to be worth asserting — that the Russian
// customer's experience is not disturbed by the Romanian work — is asserted directly instead.

check('there is exactly one response builder, and it is the premium one', () => {
  const builders = wf.nodes.filter((n) => /^Build Bot Response/.test(n.name));
  eq(builders.length, 1, 'a second response builder is present: ' + builders.map((n) => n.name).join(', '));
  eq(builders[0].name, 'Build Bot Response', 'the response builder was renamed');
  assert(/GENERATED by scripts\/build-premium-concierge\.mjs/.test(builders[0].parameters.jsCode),
    'the response builder is not the generated premium body');
});

check('a Russian customer receives the Russian machine values, unchanged, on every screen', () => {
  // The Romanian presentation is a lookup at the render boundary. For `ru` there is no lookup at
  // all — the machine value IS the label — so every RU screen must be byte-identical to the copy
  // module, and no Romanian may leak into it.
  for (const key of Object.keys(COPY)) {
    const rendered = SCREENS[key];
    if (!rendered) { continue; }
    assert(!/[ăâîșțĂÂÎȘȚ]/.test(rendered.reply_text),
      key + ' shows Romanian diacritics to a Russian customer: ' + rendered.reply_text.slice(0, 60));
  }
});

check('the locale never reaches a machine value — only the label changes', () => {
  // The same session, twice, differing only in locale. The callbacks, the state, the writes and
  // the lead readiness must be identical; only the visible text may differ.
  const cases = [
    { session: fresh(), message_text: '/start' },
    { session: fresh(), callback_data: 'p|describe' },
    { session: committed(), message_text: '/start' },
    { session: drafting(), message_text: '/start' }
  ];
  for (const c of cases) {
    const ru = run(Object.assign({}, c, { session: Object.assign({}, c.session, { language: 'ru' }) }));
    const ro = run(Object.assign({}, c, { session: Object.assign({}, c.session, { language: 'ro' }) }));
    eq(ro.debug.state_after, ru.debug.state_after, 'the locale changed the state');
    eq(ro.debug.writes, ru.debug.writes, 'the locale changed the session writes');
    eq(ro.lead_ready, ru.lead_ready, 'the locale changed lead readiness');
    const cb = (r) => (r.reply_markup.inline_keyboard || []).map((row) => row.map((b) => b.callback_data || 'web_app').join(',')).join('|');
    eq(cb(ro), cb(ru), 'the locale changed the callback contract');
    assert(ro.reply_text !== ru.reply_text, 'the Romanian reply is identical to the Russian one — nothing was localised');
  }
});
