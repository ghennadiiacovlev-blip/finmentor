...
CFO-контроль → BI/автоматизация), навигация (добавлено «Для кого»). SEO-страницы не удалялись:
...
8. **JSON payload.** `collectAnswersJson()` собирает валидный объект с секциями `source`,
   `site_version` (`production`), `lead_id`, `created_at`, `source_page`, `page_url`, `referrer`,
   `attribution`, `client`, `diagnostic`, `intake`, `automation`, а также совместимые блоки `lead`,
   `business_profile`, `main_pain`, `financial_system`, `completion`, `consent`. Персональные данные не
   логируются в консоль.
9. **Webhook по умолчанию в placeholder-режиме.** `var FINMENTOR_QUESTIONNAIRE_WEBHOOK_URL =
   "PASTE_MAKE_WEBHOOK_URL_HERE";`. Пока URL не заменён на реальный Make / n8n endpoint, форма показывает
   fallback и предлагает FINMENTOR Bot / email, ничего не отправляя наружу.
...
11. **GA4 без персональных данных.** Используются только safe-events без имени, email, телефона,
    Telegram, компании, свободного текста и полного JSON анкеты: `diagnostic_start`,
    `diagnostic_complete`, `intake_start`, `intake_submit`, `lead_submit`, `submit_questionnaire`,
    `bot_click`, `health_check_cta`. В события уходят только обобщённые параметры: `source`,
    `page_slug`, `completion_score`, `data_quality_hint`, `urgency_level`, `industry_category`,
    `desired_first_step`.
12. **Automation hints → human review.** Возле согласия добавлена строка о том, что ответы могут
