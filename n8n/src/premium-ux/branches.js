...
const COMPANY_SCREEN = { title: 'Как называется компания?', lead: 'Укажите название, которое использует команда.' };
const COMPANY_ACTIVITY_SCREEN = { title: 'Чем занимается компания?', lead: 'Кратко опишите основное направление бизнеса.' };
...
const SUCCESS = {
  ...
  next: ['FINMENTOR изучит бриф.', 'При необходимости уточним детали.', 'Мы свяжемся с вами в течение 1 рабочего дня.'],
...
const TG_COPY = {
  ...
  TG_ENTRY: {
    ...
    actions: ['Описать задачу', 'Финансовая диагностика', 'Подготовить бриф', 'Запросить встречу']
  },
  TG_FREEFORM_PROBLEM: {
    parse_mode: 'HTML',
    text: [
  ...
  TG_CONFIRM_CONTEXT: {
    ...
    labels: {
      ...
      business_activity: 'Деятельность',
      role: 'Ваша роль',
  ...
  TG_OPEN_DIAGNOSIS: {
    parse_mode: 'HTML',
    text: [
      '<b>Финансовая диагностика</b>',
      'Ответьте на несколько вопросов о текущей финансовой системе компании.',
      '<i>Результат формируется по методике FINMENTOR и проверяется консультантом.</i>'
    ],
    actions: ['Открыть диагностику']
  },
  TG_MEETING_CONFIRM: {
    parse_mode: 'HTML',
    text: [
      '<b>Запросить встречу</b>',
      'Чтобы передать запрос консультанту и связаться с вами, подтвердите обработку контактных данных.',
      '<i>Подробнее: <a href="https://finmentor.md/privacy.html">политика конфиденциальности</a>.</i>'
    ],
    actions: ['Подтвердить запрос', 'Вернуться']
  },
  TG_MEETING_REQUEST: {
    parse_mode: 'HTML',
    text: [
      '<b>Передаём запрос консультанту.</b>',
      'Подтверждение появится в этом чате после регистрации запроса.'
    ],
    actions: []
  },
  TG_UNKNOWN: {
    parse_mode: 'HTML',
    text: [
      '<b>Не удалось точно определить запрос.</b>',
      'Выберите один из вариантов ниже или опишите задачу одним сообщением.'
    ],
    actions: ['Описать задачу', 'Финансовая диагностика', 'Подготовить бриф', 'Запросить встречу']
  },
  TG_SUBMITTED: {
...
module.exports = {
  ...
  COMPANY_SCREEN, COMPANY_ACTIVITY_SCREEN, SCALE_OPTIONS, CURRENT_SETUP, DECISION_HORIZON, DOCUMENTS,
